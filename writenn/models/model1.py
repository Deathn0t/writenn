...
class TextImageGenerator(keras.callbacks.Callback):
    ...
    def build_word_list(self, num_words, max_string_len=None, mono_fraction=0.5):
        ...
        if len(tmp_string_list) != self.num_words:
            raise IOError(
                "Could not pull enough words" "from supplied monogram and bigram files."
            )
    ...
    def on_train_begin(self, logs={}):
        ...
        self.paint_func = lambda text: paint_text(
            text, self.img_w, self.img_h, rotate=False, ud=False, multi_fonts=False
        )
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if 3 <= epoch < 6:
            self.paint_func = lambda text: paint_text(
                text, self.img_w, self.img_h, rotate=False, ud=True, multi_fonts=False
            )
        elif 6 <= epoch < 9:
            self.paint_func = lambda text: paint_text(
                text, self.img_w, self.img_h, rotate=False, ud=True, multi_fonts=True
            )
        elif epoch >= 9:
            self.paint_func = lambda text: paint_text(
                text, self.img_w, self.img_h, rotate=True, ud=True, multi_fonts=True
            )
...
class VizCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        word_batch = next(self.text_img_gen)[0]
        res = decode_batch(
            self.test_func, word_batch["the_input"][0 : self.num_display_words]
        )
